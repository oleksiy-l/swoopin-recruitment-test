...
export default class StateVehicles {
  ...
  @action.bound async updateVehicles(session: any) {
    if (this.api) {
      try {
        // this.api.defaults.headers.common.Authorization = `Bearer ${session.token}`;
        const responce = await this.api?.get("vehicles");
        this.updateList(responce.data.vehicles);
      } catch {}
    }
  }
  ...
  @action.bound async setOnline(id: string) {
    try {
      ...
      this.updateVehicles(this.session);
    } catch {}
  }
