...
const PageDrivers = observer(({ location }: { location: any }) => {
  ...
  const [onlineVehicels, offlineVehicels] = useMemo(() => {
    const online: JSX.Element[] = [];
    const offline: JSX.Element[] = [];
    state.vehicles.list.forEach((vehicle: any) => {
      const vehicels = vehicle.online ? online : offline;
      vehicels.push(
        <Feature
          key={vehicle.id}
  ...
  }, [state.vehicles, mapRef.map, onMapVehicleClick]);
